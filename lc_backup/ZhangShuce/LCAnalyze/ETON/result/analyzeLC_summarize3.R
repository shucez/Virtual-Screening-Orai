...
for (file in files) {
  ...
  for (L in levels(raw$Lig)) {
    if (any(raw[grep(L,raw$Lig),]$Rec == 'A') && 
      ...
      for (S in levels(factor(raw$source[raw$Lig == L]))) {
          if (nrow(raw[raw$Lig == L,][raw[raw$Lig == L,]$source == S,]) == 3 ) {
             df1 <- rbind(df1, raw[raw$Lig == L,][raw[raw$Lig == L,]$source == S,])
          }else{
             df2 <- rbind(df1, raw[raw$Lig == L,][raw[raw$Lig == L,]$source == S,])
          }
      }
...
for (file in files) {
  ...
  write.table(df1, sep = "\t", file = paste("df",file, sep = "."))
  write.table(ddply(df1,.(Lig),summarise,B=ene[Rec=="B"],D=ene[Rec=="D"],F=ene[Rec=="F"], SRC=source[Rec=="B"]),
              sep = "\t", 
              file = paste("summ",file,sep = "."))
}
